...
ArgumentHandler::ArgumentHandler(int argc, char **argv)
    : caseweights(""), depvarname(""), fraction(1), holdout(false), memmode(MEM_DOUBLE), savemem(false), predict(""),
      predictiontype(DEFAULT_PREDICTIONTYPE), randomsplits(DEFAULT_NUM_RANDOM_SPLITS), splitweights(""),
      nthreads(DEFAULT_NUM_THREADS), predall(false), alpha(DEFAULT_ALPHA), minprop(DEFAULT_MINPROP), file(""),
      impmeasure(DEFAULT_IMPORTANCE_MODE), targetpartitionsize(0), mtry(0), outprefix("ranger_out"), probability(false),
      splitrule(DEFAULT_SPLITRULE), statusvarname(""), ntree(DEFAULT_NUM_TREE), replace(true), verbose(false),
    ...
    while (1) {
        ...
        case 'R':
            try {
                ...
                throw std::runtime_error("Illegal argument for option 'randomsplits'. Please give a positive integer. "
        ...
        case 'U':
            try {
                ...
                throw std::runtime_error("Illegal argument for option 'nthreads'. Please give a positive integer. See "
                                         "'--help' for details.");
            }
            break;
        ...
        case 'a':
            try {
                ...
                throw std::runtime_error("Illegal argument for option 'alpha'. Please give a value between 0 and 1. "
                                         "See '--help' for details.");
            }
            break;
        ...
        case 'b':
            try {
                ...
                throw std::runtime_error("Illegal argument for option 'minprop'. Please give a value between 0 and "
                                         "0.5. See '--help' for details.");
            }
            break;
        ...
        case 'i':
            try {
                ...
                throw std::runtime_error("Illegal argument for option 'impmeasure'. Please give a positive integer. "
                                         "See '--help' for details.");
            }
            break;
        ...
        case 'l':
            try {
                ...
                throw std::runtime_error("Illegal argument for option 'targetpartitionsize'. Please give a positive "
                                         "integer. See '--help' for details.");
            }
            break;
        ...
        case 'y':
            try {
                ...
                throw std::runtime_error("Illegal argument for option 'treetype'. Please give a positive integer. See "
                                         "'--help' for details.");
            }
            break;
...
void ArgumentHandler::checkArguments() {
    ...
    if (predict.empty() && depvarname.empty()) {
        throw std::runtime_error(
            "Please specify a dependent variable name with '--depvarname'. See '--help' for details.");
    }

    if (treetype == TREE_SURVIVAL && statusvarname.empty()) {
        throw std::runtime_error(
            "Please specify a status variable name with '--statusvarname'. See '--help' for details.");
    }
    if (treetype != TREE_SURVIVAL && !statusvarname.empty()) {
        throw std::runtime_error(
            "Option '--statusvarname' only applicable for survival forest. See '--help' for details.");
    }

    if (treetype == TREE_SURVIVAL && splitrule == MAXSTAT && impmeasure == IMP_GINI) {
        throw std::runtime_error("Node impurity variable importance not supported for survival forests with MAXSTAT "
                                 "splitrule. See '--help' for details.");
    }
...
void ArgumentHandler::displayHelp() {
    ...
    std::cout << "    "
              << "--version                     Print version and citation information." << std::endl;
    std::cout << "    "
              << "--verbose                     Turn on verbose mode." << std::endl;
    std::cout << "    "
              << "--file FILE                   Filename of input data. Only numerical values are supported."
              << std::endl;
    std::cout << "    "
              << "--treetype TYPE               Set tree type to:" << std::endl;
    std::cout << "    "
              << "                              TYPE = 1: Classification." << std::endl;
    std::cout << "    "
              << "                              TYPE = 3: Regression." << std::endl;
    std::cout << "    "
              << "                              TYPE = 5: Survival." << std::endl;
    std::cout << "    "
              << "                              (Default: 1)" << std::endl;
    std::cout
        << "    "
        ...
        << std::endl;
    std::cout << "    "
              << "                              Use in combination with --treetype 1." << std::endl;
    std::cout
        << "    "
    ...
    std::cout << "    "
              ...
              << std::endl;
    std::cout << "    "
              << "                              Coding is 1 for event and 0 for censored." << std::endl;
    std::cout << "    "
              << "--ntree N                     Set number of trees to N." << std::endl;
    std::cout << "    "
              << "                              (Default: 500)" << std::endl;
    std::cout << "    "
              << "--mtry N                      Number of variables to possibly split at in each node." << std::endl;
    std::cout << "    "
              << "                              (Default: sqrt(p) with p = number of independent variables)"
              << std::endl;
    std::cout << "    "
              << "--targetpartitionsize N       Set minimal node size to N." << std::endl;
    std::cout << "    "
              << "                              For Classification and Regression growing is stopped if a node reaches "
                 "a size smaller than N."
              << std::endl;
    std::cout << "    "
              << "                              For Survival growing is stopped if one child would reach a size "
                 "smaller than N."
              << std::endl;
    std::cout << "    "
              << "                              This means nodes with size smaller N can occur for Classification and "
                 "Regression."
              << std::endl;
    std::cout << "    "
    ...
    std::cout << "    "
              ...
              << std::endl;
    std::cout << "    "
              << "--write                       Save forest to file <outprefix>.forest." << std::endl;
    std::cout << "    "
              << "--predict FILE                Load forest from FILE and predict with new data. The new data is "
                 "expected in the exact same "
              << std::endl;
    std::cout << "    "
              << "                              shape as the training data. If the outcome of your new dataset is "
                 "unknown, add a dummy column."
              << std::endl;
    std::cout << "    "
              << "--predall                     Return a matrix with individual predictions for each tree instead of "
                 "aggregated "
              << std::endl;
    std::cout << "    "
              ...
              << std::endl;
    std::cout << "    "
              << "--predictiontype TYPE         Set type of prediction to:" << std::endl;
    std::cout << "    "
              << "                              TYPE = 1: Return predicted classes or values." << std::endl;
    std::cout << "    "
    ...
    std::cout << "    "
              << "                              (Default: 1)" << std::endl;
    std::cout << "    "
              << "--impmeasure TYPE             Set importance mode to:" << std::endl;
    std::cout << "    "
              << "                              TYPE = 0: none." << std::endl;
    std::cout << "    "
              << "                              TYPE = 1: Node impurity: Gini for Classification, variance for "
                 "Regression, sum of test statistic for Survival."
              << std::endl;
    std::cout << "    "
              << "                              TYPE = 2: Permutation importance, scaled by standard errors."
              << std::endl;
    std::cout << "    "
              << "                              TYPE = 3: Permutation importance, no scaling." << std::endl;
    std::cout << "    "
              << "                              TYPE = 5: Corrected node impurity: Bias-corrected version of node "
                 "impurity importance."
              << std::endl;
    std::cout << "    "
              << "                              (Default: 0)" << std::endl;
    std::cout << "    "
              << "--noreplace                   Sample without replacement." << std::endl;
    std::cout << "    "
              << "--fraction X                  Fraction of observations to sample. Default is 1 for sampling with "
                 "replacement "
              << std::endl;
    std::cout << "    "
              << "                              and 0.632 for sampling without replacement." << std::endl;
    std::cout << "    "
              << "--splitrule RULE              Splitting rule:" << std::endl;
    std::cout << "    "
              << "                              RULE = 1: Gini for Classification, variance for Regression, logrank "
                 "for Survival."
              << std::endl;
    std::cout
        << "    "
        ...
        << std::endl;
    std::cout << "    "
              << "                              RULE = 3: AUC (ignore ties) for Survival, not available for "
                 "Classification and Regression."
              << std::endl;
    std::cout << "    "
              << "                              RULE = 4: MAXSTAT for Survival and Regression, not available for "
                 "Classification."
              << std::endl;
    std::cout << "    "
              << "                              RULE = 5: ExtraTrees for all tree types." << std::endl;
    std::cout << "    "
              << "                              (Default: 1)" << std::endl;
    std::cout << "    "
              << "--randomsplits N              Number of random splits to consider for each splitting variable "
                 "(ExtraTrees splitrule only)."
              << std::endl;
    std::cout << "    "
              << "--alpha VAL                   Significance threshold to allow splitting (MAXSTAT splitrule only)."
              << std::endl;
    std::cout << "    "
              << "--minprop VAL                 Lower quantile of covariate distribtuion to be considered for "
                 "splitting (MAXSTAT splitrule only)."
              << std::endl;
    std::cout << "    "
              << "--caseweights FILE            Filename of case weights file." << std::endl;
    std::cout << "    "
              << "--holdout                     Hold-out mode. Hold-out all samples with case weight 0 and use these "
                 "for variable "
              << std::endl;
    std::cout << "    "
              << "                              importance and prediction error." << std::endl;
    std::cout << "    "
              << "--splitweights FILE           Filename of split select weights file." << std::endl;
    std::cout
        << "    "
        << "--alwayssplitvars V1,V2,..    Comma separated list of variable names to be always considered for splitting."
        << std::endl;
    std::cout << "    "
              << "--nthreads N                  Set number of parallel threads to N." << std::endl;
    std::cout << "    "
              << "                              (Default: Number of CPUs available)" << std::endl;
    std::cout << "    "
              << "--seed SEED                   Set random seed to SEED." << std::endl;
    std::cout << "    "
              << "                              (Default: No seed)" << std::endl;
    std::cout << "    "
              << "--outprefix PREFIX            Prefix for output files." << std::endl;
    std::cout << "    "
              << "--memmode MODE                Set memory mode to:" << std::endl;
    std::cout << "    "
              << "                              MODE = 0: double." << std::endl;
    std::cout << "    "
              << "                              MODE = 1: float." << std::endl;
    std::cout << "    "
              << "                              MODE = 2: char." << std::endl;
    std::cout << "    "
              << "                              (Default: 0)" << std::endl;
    std::cout << "    "
              << "--savemem                     Use memory saving (but slower) splitting mode." << std::endl;
    std::cout << std::endl;
...
void ArgumentHandler::displayVersion() {
    ...
    std::cout << "Wright, M. N. & Ziegler, A. (2017). ranger: A Fast Implementation of Random Forests for High "
